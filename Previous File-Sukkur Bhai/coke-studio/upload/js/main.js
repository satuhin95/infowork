...
;(function(window) {
    ...
    // var touchArea = document.querySelector(".cokeplayer"),
    //     startX,
    //     startY,
    //     dist,
    ...
    //   if (isSwiped) {
    //     getXY(event);
    //     dist = Math.abs(mouseX - initialX) ;
    //     // get total dist traveled by finger while in contact with surface
    //     elapsedTime = new Date().getTime() - startTime; 
    //     ;// get time elapsed
    //     // check that elapsed time is within specified, horizontal dist traveled >= threshold, and vertical dist traveled <= 100
    //     var swiperightBol = (elapsedTime <= allowedTime && dist >= threshold && Math.abs(mouseY - initialY) <= 100);        
        
    //     if (swiperightBol === true) {
	...
	RecordSlideshow.prototype._initEvents = function() {
        ...
        var touchArea = document.querySelector(".is_wrapper"),
            startX,
            startY,
            dist,
            threshold = 110, //required min distance traveled to be considered swipe
        ...
        if (isSwiped) {
            ...
            ;// get time elapsed
            // check that elapsed time is within specified, horizontal dist traveled >= threshold, and vertical dist traveled <= 100
            var swiperightBol = (elapsedTime <= allowedTime && dist >= threshold && Math.abs(mouseY - initialY) <= 100 && self._state());        
	 ...
	 * Load the record.
	 */
	RecordSlideshow.prototype._state = function() {
		var self = this;
		
        var currentRecord = this.records[this.current];
        var result = classie.has( currentRecord.wrapper, 'single--current' );
	...
	var views = {
		...
		slideshow, turntable;
        var number_total = document.querySelectorAll('.number__total');
        number_total.forEach(function(e){
            e.innerHTML = lps.length;
        });
	...
	function initEvents() {
        ...
        setTimeout(function() { slideshow.start(0);}, 80);
	...
	function showExpander(position, callback) {
		dynamics.css(expanderEl, { opacity: 1, left : position.x, top : position.y, backgroundColor : '#f70008', scale : 0 });
