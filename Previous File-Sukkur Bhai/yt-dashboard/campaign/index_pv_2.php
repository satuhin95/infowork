...
if (!isset($_SESSION['username']) || !$_SESSION['username']) {
  ...
  if ($userInfoRes['user_id'] == $campHead['user_id']) {
    ...
    $deviceTable = "";
    $deviceTableExist = 'false';
    foreach ($deviceResults as $row) {
      $deviceTableExist = 'true';
    ...
    // ***************************** Location **********************************
    $stmt = $db->prepare("SELECT * FROM locAdStats WHERE camp_id = $campID");
    $stmt->execute();
    $locResults = $stmt->fetchAll(PDO::FETCH_ASSOC);


    $totalLocImpressions = 0;
    $totalLocClicks = 0;
    $totalLocViews = 0;

    $locTable = "";
    $locTableExist = 'false';
    foreach ($locResults as $row) {
      $locTableExist = 'true';
      $impressions = number_format(($row["loc_impressions"]));
      $clicks = number_format(($row["loc_clicks"]));
      $ctr = number_format(($row["loc_clicks"] / $row["loc_impressions"]) * 100, 3);

      $b = <<<STR
        <tr>
        <td style="max-width: 100px;min-width: 50px;" class="pdLeft20">{$row["loc_city"]}</td>
        <td class=" d-xl-table-cell">{$impressions}</td>
        <td class=" d-xl-table-cell">{$clicks}</td>
        <td class=" d-xl-table-cell">{$ctr}%</td>
      </tr>
      ...
      $totalLocImpressions += $row["loc_impressions"];
      $totalLocClicks += $row["loc_clicks"];
    }
    // end Loc ***********
...
<body>
    ...
    <div class="main">
      ...
      <main class="content">
        <div class="container-fluid p-0">
          ...
          <!-- Location Stats -->
          <div class="row locTableRow">
            <div class="col-12  d-flex">
              <div class="card flex-fill" style="overflow-x: auto;">
                <div class="card-header">

                  <h5 class="card-title mb-0" style="font-size: 20px;">Location Stats</h5>
                </div>
                <table class="table table-hover table-striped my-0 locTable_full" style="max-height: 500px !important; overflow: auto;">
                  <thead>
                    <tr>
                      <th class=" d-xl-table-cell pdLeft20">Location</th>
                      <th class=" d-xl-table-cell">Impressions</th>
                      <th class=" d-xl-table-cell">Clicks</th>
                      <th class=" d-xl-table-cell">CTR</th>
                    </tr>
                  </thead>
                  <tbody class="locTable_body">
                    <?php echo $locTable; ?>
                  </tbody>
                  <tfoot>
                    <tr>
                      <th colspan="1" class="pdLeft20">Total</th>
                      <th><?php echo number_format($totalLocImpressions); ?></th>
                      <th><?php echo number_format($totalLocClicks); ?></th>
                      <th></th>
  ...
  <script>
    ...
    let deviceTableExist = <?php echo $deviceTableExist; ?>;
    ...
    let ageTableExist = <?php echo $ageTableExist; ?>;
    // console.log(ageTableExist)
    if (ageTableExist) {
    ...
    // let locTableExist = true;
    let locTableExist = <?php echo $locTableExist; ?>;
    // console.log(locTableExist)
    if (locTableExist) {
      document.querySelector('.locTableRow').style.display = 'flex';
    } else {
      document.querySelector('.locTableRow').style.display = 'none';
    }
  </script>
</body>
